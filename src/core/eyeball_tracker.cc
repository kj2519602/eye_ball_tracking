...
void EyeballTracker::Init(const Ellipse& pupil) {
    ...
    if (!occupation_bins_[idx]) {
        DualCircle dual_circle;
        if (!camera_.Unproject(pupil, options_.pupil_radius, &dual_circle)) {
            return;
        }
...
void EyeballTracker::EstimateEyeSphere() {
    ...
    camera_.Unproject(intersect_lines(lines), options_.eye_distance, &eye_.sphere.center);
     // Find eyeball radius.
