...
Scalar Ellipse::Distance(Scalar x, Scalar y) const {
    // Prepare constant.
    Scalar cos_angle, sin_angle, r00, r01, r10, r11;
    cos_angle = std::cos(angle);
    sin_angle = std::sin(angle);
    r00 = cos_angle;
    r01 = sin_angle;
    r10 = -major_radius * sin_angle / minor_radius;
    r11 = major_radius * cos_angle / minor_radius;
    // Calculate distance.
    Scalar rxt = r00 * (x - cx) + r01 * (y - cy);
    Scalar ryt = r10 * (x - cx) + r11 * (y - cy);
    return major_radius - std::sqrt(rxt * rxt + ryt * ryt);
}
