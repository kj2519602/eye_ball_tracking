...
DualCircle Camera::Unproject(const Ellipse& ellipse, const Scalar radius) const {
...
Vector3 Camera::Unproject(const Vector2& point2d, const Scalar distance) const {
    const auto x = (point2d(0) - cx_) * distance / focal_length_;
    const auto y = (point2d(1) - cy_) * distance / focal_length_;
    const auto z = distance;
