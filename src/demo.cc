...
std::size_t get_ordered_frames(const std::string& input_path, std::vector<cv::String>& frame_files) {
    cv::utils::fs::glob(input_path, "*.tiff", frame_files); 
...
void run(const std::string& input_path, int fps = 50) {
    ...
    while(1) {
        ...
        const auto eye = pupil_tracker.Track(frame);
        ...
        if (pupil_tracker.IsInitialized() && eye.is_valid) {
            ...
            cv::ellipse(vis, rect, cv::Scalar(0, 255, 255), 1);
            // Draw eye ball.
            cv::ellipse(vis, eye.eye_ball.Rect(), cv::Scalar(255, 255, 0), 1);
            // Draw pupil center.
            auto pupil = camera.Project(eye.circle.position);
            cv::Point2f pupil_center(pupil[0], pupil[1]);
            cv::circle(vis, pupil_center, 2, cv::Scalar(0, 0, 255));
            // Draw eye ball center.
            auto center = eye.eye_ball.Center();
            cv::Point2f eyeball_center(center[0], center[1]);
            cv::drawMarker(vis, eyeball_center, cv::Scalar(0, 255, 0), cv::MARKER_CROSS, 10, 1);
            // Draw gaze vector.
            auto gaze_end = camera.Project(eye.circle.position + (eye.circle.norm * 20)); 
            cv::line(vis, pupil_center, cv::Point2f(gaze_end[0], gaze_end[1]),
                cv::Scalar(0, 0, 255), 1);
            cv::line(vis, eyeball_center, pupil_center, cv::Scalar(0, 255, 0), 1);
            // Log angular results.
            std::cout << "------------------------------" << std::endl;
